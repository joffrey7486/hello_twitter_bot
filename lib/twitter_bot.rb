...
Dotenv.load(".env")# Ceci appelle le fichier .env (situé dans le même dossier que 
...
def hello_journalist(client)
  ...
  journalists_selected = journalist_list.shuffle[0..1]
  journalists_selected.each do |journalist|
      client.update ("Hello #{journalist} ! Merci pour votre dernier article et votre travail !! Et dieu bénisse @the_hacking_pro")
  end
end
...
def follow_hello(client)
  client.search('#bonjour_monde').take(5).each do |tweet|
    client.follow(tweet.user).favorite(tweet)
  end
end

def like_and_follow_hello(client_streaming)
  client_streaming.search('#bonjour_monde').take(5).each do |tweet|
    client_streaming.follow(tweet.user)
  end
end

def stream_tweets(client_streaming)
  client_streaming.filter(track: "#bonjour_monde") do |object|
    puts object.text if object.is_a?(Twitter::Tweet)
    client = login_twitter
    client.favorite(object)
    # client_streaming.favorite(object)
  end
end
...
def perform
    # client = login_twitter
    # hello_journalist(client)
    # like_hello(client)
    # follow_hello(client)
    client_streaming = login_twitter_streaming
    stream_tweets(client_streaming)
